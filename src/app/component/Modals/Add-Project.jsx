...
const ProjectModal = ({ project, token, onClose, onSuccess }) => {
    ...
    // Get project context to access selected project ID
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (!isEditMode) {
                // Create new project - include selectedProject ID if available
                const requestData = {
                    ...formData,
                    ...(selectedProject && { parentProjectId: selectedProject._id })
                };
                ...
                // Update existing project - use the project ID from context if available, otherwise use the prop
                const projectId = selectedProject?._id || project._id;
    ...
    return (
        <div
            className={`fixed inset-0 transition-all duration-300 ease-out flex items-center justify-center p-4 z-50 ${isVisible ? 'bg-black bg-opacity-50' : 'bg-opacity-0'
                }`}
        >
            {/* Success Alert */}
            {successMessage && (

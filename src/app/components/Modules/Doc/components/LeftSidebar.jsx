...
const LeftSidebar = ({ leftSidebarOpen, activeTab, setActiveTab }) => {
    const [showDocList, setShowDocList] = useState(() => {
        const saved = localStorage.getItem('showDocList');
        return saved ? JSON.parse(saved) : false;
    });

    useEffect(() => {
        localStorage.setItem('showDocList', JSON.stringify(showDocList));
    }, [showDocList]);
    ...
    const leftMenuItems = [
        ...
        { id: 'script', label: 'Add Script', icon: FiCode },
        { id: 'sheets', label: 'Add Sheets', icon: FiBarChart2 },
    ];
    ...
    const quickActions = [
        ...
        { label: 'Export .pdf', icon: FiDownload },
        { label: 'Export .docx', icon: FiFile },
        { label: 'Export .html', icon: Code },
